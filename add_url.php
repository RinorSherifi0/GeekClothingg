...
if (isset($_POST['add_url'])) {
  ...
  $query = "SELECT * FROM urls WHERE title='$title'";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    $query = "INSERT INTO urls (title, url, user_id, qrcode) VALUES ('$title', '$url', '$username', '$randomcnt')";
    $result = mysqli_query($connection, $query);

    if ($result) {
      
      $insert_id = mysqli_insert_id($connection);
       $query = "INSERT INTO url_count (url_id, count) VALUES ('$insert_id', '0')";
    $result = mysqli_query($connection, $query);

      $query = "INSERT INTO redirects (qr_id,redirect_url,qrcode) VALUES ('$insert_id','$url','$randomcnt')";
      $result = mysqli_query($connection, $query);

      // Get the ID of the inserted URL
      $data =  "artwear.co.za/r.php?url_id=$insert_id";
...
<div class="container-box">
 ...
 <?php if($insertedUrl) {?>
  ...
  <?php } ?>
  <?php if($alreadyUsed) {?>
    <div class="success-banner2" style="display: none; background-color: #ffc0c0; padding: 10px; border-radius: 10px;">
    <p>The title is used please use a different one.</p>
  </div>
  <?php } ?>
    <div class="susi-fields-wrapper">
      <fieldset>
        <center>
        <h4 class="text" for="new_url">Create new QR code</h4></center>
        <label class="text">Title</label>
        <input class="text" type="text" id="title" name="title"  style="text-align:center;" tabindex="3" autocorrect="off" autocapitalize="none"/>
        <label class="text">URL</label>
        <input class="text" type="text" id="url" name="url"  style="text-align:center;" tabindex="3" autocorrect="off" autocapitalize="none"/>
